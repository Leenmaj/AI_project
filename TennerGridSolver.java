public class TennerGridSolver {
    ...
    public boolean backtrack(int row, int col) {
        if (row == rows) return verifySums();
        if (col == 10) return backtrack(row + 1, 0);
    ...
    public boolean isSafe(int row, int col, int value) {
        if (row == 0 && value == sums[col]) return false;
        ...
        for (int[] d : directions) {
            ...
            if (drow >= 0 && drow < rows && dcol >= 0 && dcol < 10) {
                if (grid[drow][dcol].value == value) return false;
            }
        }
        if (row > 0 && grid[row - 1][col].value == value) return false;
        // checking the cell directly below onlyy if not in the last row
        if (row < rows - 1 && grid[row + 1][col].value == value) return false;
    
        // making sure adding  doesn't exceed the goal sum for the column
        int tempSum = value;
        for (int i = 0; i < rows; i++) {
            if (i != row) tempSum += (grid[i][col].value != -1) ? grid[i][col].value : 0;
        }
        if (tempSum > sums[col]) return false;
        ...
        // calculate the maximum possible sum for the column assuming remaining cells take the highest value which is9
        maxPossSum += remainingCells * 9;
        if (maxPossSum < sums[col]) return false; // if even the max possible sum is less than the target its not safe
